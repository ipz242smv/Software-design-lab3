...
Sep("1. ПАРСИНГ КНИГИ В LIGHTHTML (перші 8 елементів)");

var root = BookParser.Parse(bookText);
var stats = BookParser.CollectTagStats(root);

Console.WriteLine($"Всього дочірніх елементів у <div.book>: {root.ChildCount}");
Console.WriteLine("\nСтатистика тегів:");
foreach (var (tag, count) in stats.OrderByDescending(x => x.Value))
    Console.WriteLine($"  <{tag}>  →  {count} вузлів");

Console.WriteLine("\n\n\n");
foreach (var child in GetChildren(root))
{
    string html = child.OuterHTML;
    string preview = html.Length > 90 ? html[..90] + "…" : html;
    Console.WriteLine($"  {preview}");
}

Console.WriteLine("\n\n\n");

Sep("2. АНАЛІЗ ПАМ'ЯТІ ДЕРЕВА");
...
long logicalSize = MemoryMeter.Measure(root);

Console.WriteLine($"Вузлів у дереві (всього):   {root.ChildCount + 1}");
Console.WriteLine($"Логічний розмір дерева:      {logicalSize,12:N0} байт  ({logicalSize / 1024.0:F1} КБ)");
Console.WriteLine($"GC.GetTotalMemory (до):      {before,12:N0} байт");
Console.WriteLine($"GC.GetTotalMemory (після):   {after,12:N0} байт");

Sep("3. ЛЕГКОВАГОВИК: ПОРІВНЯННЯ ПАМ'ЯТІ");

int nodeCount = root.ChildCount;

long styleWithoutFlyweight = 0;
foreach (var (tag, count) in stats)
{
    long perObject = 16
                   + 8
                   + MemoryMeter.MeasureString(tag)
                   + 4
                   + 4;
    styleWithoutFlyweight += perObject * count;
}

long poolSize = 0;
foreach (var (tag, _) in stats)
{
    poolSize += 16
             + 8
             + MemoryMeter.MeasureString(tag)
...
poolSize += 48 + stats.Count * 32L;

long saved = styleWithoutFlyweight - poolSize;
double percent = styleWithoutFlyweight > 0
    ? saved * 100.0 / styleWithoutFlyweight
    : 0;

Console.WriteLine($"Унікальних стилів (Flyweight-пул): {ElementStyleFactory.CachedCount}");
Console.WriteLine();
ElementStyleFactory.PrintPool();
Console.WriteLine();
Console.WriteLine($"Вузлів-елементів у дереві:          {nodeCount,8}");
Console.WriteLine();
Console.WriteLine($"Пам'ять на ElementStyle БЕЗ FW:    {styleWithoutFlyweight,10:N0} байт");
Console.WriteLine($"Пам'ять на пул Flyweight:           {poolSize,10:N0} байт");
Console.WriteLine($"Економія завдяки Flyweight:         {saved,10:N0} байт  ({percent:F1}%)");

// ════════════════════════════════════════════════════════════════════════════
Sep("4. ДЕМОНСТРАЦІЯ СПІЛЬНОСТІ FLYWEIGHT-ОБ'ЄКТІВ");

var p1 = new LightElementNode("p");
var p2 = new LightElementNode("p");
var h1 = new LightElementNode("h1");
var h2 = new LightElementNode("h2");
var bq = new LightElementNode("blockquote");

var styleField = typeof(LightElementNode)
    .GetField("_style",
        System.Reflection.BindingFlags.NonPublic |
        System.Reflection.BindingFlags.Instance)!;

object? style_p1 = styleField.GetValue(p1);
object? style_p2 = styleField.GetValue(p2);
object? style_h1 = styleField.GetValue(h1);
object? style_h2 = styleField.GetValue(h2);

Console.WriteLine($"p1._style  is  p2._style  →  {ReferenceEquals(style_p1, style_p2)}   (очікується: True)");
Console.WriteLine($"p1._style  is  h1._style  →  {ReferenceEquals(style_p1, style_h1)}  (очікується: False)");
Console.WriteLine($"h1._style  is  h2._style  →  {ReferenceEquals(style_h1, style_h2)}  (очікується: False)");

Console.WriteLine($"\nЗагальна кількість об'єктів у Flyweight-пулі: {ElementStyleFactory.CachedCount}");
Console.WriteLine("(Не зростає з кількістю вузлів — тільки з кількістю унікальних комбінацій тег/display/closing)");

Sep("5. ПЕРЕВІРКА ПРАВИЛ ПАРСИНГУ (перший рядок + приклади)");

var firstChild = GetChildren(root).First();
Console.WriteLine($"Перший рядок → тег: <{(firstChild as LightElementNode)?.TagName}> (очікується: h1)");

var firstH2 = GetChildren(root).OfType<LightElementNode>().FirstOrDefault(e => e.TagName == "h2");
Console.WriteLine($"Перший <h2> (< 20 символів): {firstH2?.OuterHTML[..Math.Min(60, firstH2.OuterHTML.Length)]}");

var firstBq = GetChildren(root).OfType<LightElementNode>().FirstOrDefault(e => e.TagName == "blockquote");
Console.WriteLine($"Перший <blockquote> (пробіл): {firstBq?.OuterHTML[..Math.Min(70, firstBq.OuterHTML.Length)]}…");
...
static IEnumerable<LightNode> GetChildren(LightElementNode node)
{
